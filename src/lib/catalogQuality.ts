...
const getOfferClassificationText = (product: CatalogQualityInput) =>
  cleanEncodingNoise(
    [
      product.name,
      product.description,
...
const removeLeadingManufacturer = (text: string, manufacturer?: string) => {
  const brand = cleanEncodingNoise(manufacturer || "").toLowerCase();
